...
- **Bilinen sınırlama:** Bu ortamda (Windows) `chromium-cli`/Playwright yok VE bu denetim sırasında **fiziksel bir Android cihaz da bağlı değildi** (`adb devices` boş, `flutter devices` yalnızca Windows desktop + Chrome/Edge web hedeflerini gösterdi). Kullanıcı "gerçek cihazda USB bağlı deneyerek doğrula" istedi ama bu oturumda hiçbir cihaz erişilebilir değildi — mobil taraftaki tüm değişiklikler yalnızca `flutter analyze`/`flutter test` ile doğrulanabildi, gerçek dokunma/UI akışıyla test EDİLEMEDİ. Kullanıcının bir cihaz bağlayıp elle doğrulaması önemle öneriliyor — özellikle madde (a)'nın yeni "Komutlar" bölümü, (c)'nin yeni bottom sheet'i ve tekli atama seçicisinin gerçek dokunmatik akışı.
