...
### [22 Ağustos 2026] - Web + Mobil: Komut paleti, bildirim güncelleme, presence paneli, atanan-kişi birleştirme, performans paketi (kullanıcı talebiyle)

> Kullanıcı, önceki oturumlarda eklenen özelliklerin (global arama, presence, çoklu atama, dashboard) üzerine 6 maddelik yeni bir istek listesi bildirdi. Web tarafındaki en riskli/iş-mantığı ağırlıklı maddeler (çoklu atama hatası, bildirim güncelleme, backend performans) doğrudan yapıldı; komut paleti, presence paneli, kalan web performans işleri ve mobil eşitleme paralel arka plan ajanlarına verilip sonuçlar gözden geçirilip entegre edildi.

**1. Global arama (Cmd/Ctrl+K) artık uygulama içi EYLEMLERİ de buluyor.** Önceden yalnızca içerik (görev/proje/üye/not) buluyordu. `cmdk` tabanlı palete yeni bir "Komutlar" bölümü eklendi: "Yeni Görev Oluştur" (yalnızca bir proje sayfasındayken — hangi projeye ekleneceği belirsiz olmasın diye bağlama duyarlı), "Üye Davet Et", "Yeni Workspace Oluştur", "Temayı Değiştir (Açık/Koyu)". Bu eylemlerin modalları (`CreateTaskModal`, sidebar'daki `InviteMemberModal`/`CreateWorkspaceModal`) hep sayfa-yerel `useState` ile açılıyordu, global bir modal yöneticisi hiç yoktu — büyük bir refactor yerine URL'i durum taşıyıcı olarak kullanan hafif bir köprü kuruldu (yeni `frontend/src/lib/command-actions.ts`): palet hedef sayfaya `?cmd=create-task|invite-member|create-workspace` ile gidiyor, ilgili bileşen mount olduğunda bu parametreyi okuyup kendi modalını açıyor ve parametreyi `router.replace` ile temizliyor. Tema değişimi navigasyon gerektirmediği için doğrudan `next-themes`'in `setTheme`'i çağrılıyor.

**2. BUG: Bildirimler düzgün güncellenmiyordu — iki bağımsız kök neden bulundu.**
- **(a) Web'de Supabase JWT hiç yenilenmiyordu.** Kod taraması: `frontend/src/lib/supabase/client.ts`'teki tüm istemciler `autoRefreshToken: false`; projede `refreshSession`/`TOKEN_REFRESHED`/periyodik bir yenileme mekanizması **hiç yoktu** (yalnızca backend'de mobilin kullandığı `/auth/refresh` ucu vardı, web hiç çağırmıyordu). Sonuç: ~1 saatlik Supabase JWT süresi dolunca `createAuthedRealtimeClient()`'ın kurduğu bildirim Realtime aboneliği sessizce doğrulanamaz hale geliyordu — `.subscribe()`'da durum/hata callback'i de olmadığından bu hiç görünür değildi. Düzeltme: yeni `frontend/src/lib/auth-token-refresh.ts` — JWT `exp`'ine göre kendi kendini zamanlayan bir döngü, süresi dolmadan ~2dk önce (ve sekme arka plandan dönünce `visibilitychange` ile) var olan `/auth/refresh` ucunu çağırıp oturumu güncelliyor; yeni `AuthTokenRefreshProvider` dashboard shell'e bağlandı. Yenileme sonrası bir `window` event'i ile `invite-notifications-menu.tsx`'in Realtime effect'i (artık bir `tokenVersion` state'ine bağımlı) eski client'ı söküp taze token'la yeniden abone oluyor; `.subscribe()`'a da `CHANNEL_ERROR`/`TIMED_OUT` için log eklendi (önceden hiç izlenmiyordu).
- **(b) Backend: workspace daveti bildirimi gerçek bildirim servisini atlıyordu.** `WorkspaceService.invite()` (`workspace.service.ts`), davet bildirimini `NotificationService.create()` yerine doğrudan `client.from('notifications').insert(...)` ile yazıyordu — bu satır DB'ye yazıyordu (web'in Supabase Realtime dinleyicisi görüyordu) ama `NotificationGateway.emitToUser()`'ı hiç tetiklemiyordu, yani **mobil (Socket.IO) davet bildirimini asla canlı almıyordu**, yalnızca elle yenilemede görüyordu. `WorkspaceModule`'e `NotificationModule` eklenip `WorkspaceService`'e `NotificationService` inject edildi, doğrudan insert `notificationService.create(...)` çağrısına çevrildi.
- **(c) DAHA BÜYÜK bir kök neden — canlı testte bulundu, planlanan kapsamın dışında ama doğrudan aynı belirtiyi üretiyordu.** Madde (b)'nin düzeltmesini canlıda gerçek bir istekle doğrularken (`POST /workspace/:id/invite`), çağrı **RLS hatasıyla patladı**: `"new row violates row-level security policy for table \"notifications\""`. Araştırma: `NotificationService`'in **TÜM** metodları (`create`, `findAllForUser`, `markAsRead`, `markAllAsRead`, `respondToClaim`, `respondToDeletion`, `getWorkspaceAdminIds`) paylaşılan singleton `this.supabaseService.getClient()`'ı kullanıyordu — bu istemcinin **hiçbir kullanıcı oturumu yok** (`auth.uid()` = NULL), ama `notifications` tablosunun RLS politikaları (`notifications_insert_workspace_members`, `notifications_select_own`, `notifications_update_own`) tamamen `auth.uid()`'e dayanıyor. Sonuç: INSERT'ler RLS hatasıyla patlıyor, SELECT/UPDATE'ler ise hata vermeden **sessizce sıfır satır** dönüyor (RLS eşleşmezse hata değil, boş sonuç — bu yüzden aylarca fark edilmemiş olabilir). Bu, mobil üzerinden bildirim listesi çekmenin ve okundu işaretlemenin de sessizce bozuk olabileceği anlamına geliyordu. Düzeltme: dosyadaki tüm `getClient()` çağrıları `WorkspaceRoleGuard`'ın zaten kullandığı `getAdminClient() ?? getClient()` desenine çevrildi (yetkilendirme zaten controller/guard katmanında yapıldığı için güvenli). **Aynı kök neden `task.service.ts`'teki `listAssignees`/`setAssignees`'te de vardı** (madde 5'in canlı testinde ayrıca bulundu, aynı şekilde düzeltildi).
  - **ÖNEMLİ — bu bir denetim, bir kapatma değil:** `grep -rn "this.supabaseService.getClient()"` backend genelinde **14 dosyada 54 kullanım** buluyor (`activity-log`, `admin`, `auth`, `comment`, `dashboard`, `file`, `invitation`, `note`, `personal`, `progress-report`, `project`, `task`, `workspace` servisleri). Bugün yalnızca canlı testle GERÇEKTEN kırık olduğu kanıtlanan iki dosya (`notification.service.ts`, `task.service.ts`'in atama metodları) düzeltildi. Diğer 12 dosyadaki kullanımların bir kısmı zararsız olabilir (RLS'i daha az kısıtlayıcı tablolara dokunuyor olabilirler, ya da sorgu zaten `.eq('user_id', ...)` gibi açık bir filtre içeriyor olabilir) ama bu doğrulanmadı — **ayrı, kapsamlı bir oturumda tek tek denetlenmesi öneriliyor.** Bkz. `CLAUDE.md` Bölüm 4/5 (bilinen eksikler).
- Ayrıca (bilinç kapsamı dışı, not edildi): yorumlar ve ilerleme raporları hiçbir zaman bildirim üretmiyor — ne backend'de ne web'de bu akışlar `NotificationService`'e dokunuyor. Bu bir "kablo kopukluğu" değil, gerçek bir eksik özellik; bu oturumun kapsamı dışında bırakıldı.

**3. Dashboard'da "Aktif Üyeler" kartına tıklayınca kimlerin online olduğunu gösteren panel.** Yeni `frontend/src/components/analytics/online-members-popover.tsx` — `useWorkspacePresence()`'tan gelen `onlineUserIds` (yalnızca ID, ad/avatar yok) ile, panel ilk açıldığında (sayfa yüklemesinde DEĞİL — performansı etkilememesi için lazy, `client-cache`'teki paylaşılan üye önbelleğini kullanarak) çekilen `getWorkspaceMembers()` sonucunu eşleştirip avatar+ad listesi gösteriyor. Görsel olarak `invite-notifications-menu.tsx`'teki `DropdownMenu` desenini takip ediyor.

**4. Görev atama arayüzündeki ikilik tek bileşende birleştirildi.** Web'de hem `CreateTaskModal` hem `TaskDetailSheet`'te tekli `assignee_id` `<select>`'i ile ayrı bir çoklu `task_assignees` checkbox listesi yan yana duruyordu (aynı kavramın iki ayrı, kısmen çelişen widget'ı — TaskDetailSheet'te ikisi arasında hiç dışlama da yoktu, aynı kişi hem birincil hem ek atanan olarak iki kez eklenebiliyordu). Yeni `frontend/src/components/task/assignees-field.tsx`: TEK bir çoklu-seçim checklist'i, sıralı bir `selectedIds: string[]` state'i üzerinden çalışıyor — index 0 her zaman birincil atanan (`assignee_id`), geri kalanı ek atanan (`task_assignees`). Admin olmayanlar için (`canMultiSelect=false`) yeni bir kişi işaretlemek TÜM seçimi o kişiyle değiştiriyor (radyo davranışı) — önceki "primary her zaman düzenlenebilir, ek atananlar yalnızca admin" izin modeli korundu. TaskDetailSheet'te birincil ve ek atananların ayrı ayrı iki "Kaydet" akışı vardı; tek bir "Atananları Kaydet" butonuna indirgendi (birincil değiştiyse `updateTask`, admin ise ek atananlar için `setTaskAssignees` — ikisi de aynı tıklamada). Mobilde de aynı tasarım birebir uygulandı (aşağıya bkz.).

**5. BUG: Birden fazla kişiye görev atama web'de çalışmıyordu — kayıt "başarılı" dönüyor ama liste hep boş kalıyordu.** Kök neden prod'da doğrulandı: `frontend/src/app/actions/task-assignees.ts`'teki `getTaskAssignees()`, `task_assignees.user_id`'yi `profiles:user_id(...)` embed'iyle çekiyordu — ama bu sütun şemada `profiles`e değil `auth.users`e FK'lı, yani PostgREST bu ilişkiyi **her zaman** `PGRST200 - Could not find a relationship` hatasıyla reddediyordu (12 Ağustos'ta global aramadaki aynı sınıf hatayla birebir aynı kök neden — `global-search.ts`'te zaten düzeltilmişti ama bu dosyaya hiç taşınmamıştı). Hatayı ele alan kod da `error.message.includes("task_assignees")` gibi aşırı geniş bir kontrol kullanıyordu — bu PGRST200 hatası da (mesajda tablo adı geçtiği için) "migration henüz çalıştırılmadı" sanılıp sessizce boş bir başarı olarak yutuluyordu. `setTaskAssignees()` (delete+insert) kendisi hatasız çalışıyordu (embed içermiyordu) ama sonunda çağırdığı `getTaskAssignees()` her zaman boş dönüyordu — kullanıcı "kaydedildi" toast'ı görüyor ama seçici hep 0 kişi gösteriyordu. Düzeltme: embed kaldırıldı, profiller `loadProfilesByIds` ile ayrı sorgulanıyor (global aramadaki gibi); hata sınıflandırması Postgres error code'una (`42P01`/`42703`) veya kesin "relation ... does not exist" mesaj eşleşmesine dayandırıldı, artık gerçek RLS/izin hatalarını yanlış sınıflandırmıyor. Backend'deki (mobil/NestJS) eşdeğer `isMissingAssigneesTable` kontrolü de aynı şekilde hassaslaştırıldı (backend zaten embed kullanmıyordu, o yüzden bu spesifik bug'dan etkilenmiyordu ama aynı geniş-eşleşme riskini taşıyordu) — ayrıca `PUT .../assignees` uç noktasına artık gerçek bir DTO (`SetTaskAssigneesDto`, `class-validator` ile `user_ids: string[]` dizi/UUID doğrulaması) var; önceden `@Body() body: { user_ids: string[] }` yalnızca bir TS tipiydi, çalışma zamanında hiç doğrulanmıyordu.

**6. Performans.**
- Frontend: `resolveWorkspaceRole` React `cache()` ile sarıldı — bir `/project/[id]` sayfa yüklemesinde 3 ayrı yerden (page.tsx, `getProjectById`, `getProjectTasks`) tekrar tekrar çağrılan bu fonksiyon artık istek başına 1 kez çalışıyor. `getWorkspaceAnalytics`'te `loadProjectNames`+`enrichWorkload` ve `countWorkspaceMembers`'ın ilk iki bağımsız sorgusu artık `Promise.all` ile paralel. `getMemberVisibleProjectIds`'teki `taskAttempts` fallback döngüsünde eksik olan `break` eklendi (yanındaki `projectAttempts` döngüsü zaten ilk başarıda duruyordu, bu unutulmuştu) — Member rolündeki her kullanıcının proje/görev sayfası yüklemesinde gereksiz yere 4 fallback sorgusunun hepsi çalışıyordu.
- Backend: `TaskService.isWorkspaceAdmin` (görev listesi ve diğer birçok admin kontrolünde çağrılıyor) iki bağımsız sorguyu sıralı çalıştırıyordu (`WorkspaceRoleGuard`'daki paralel eşdeğerinin aksine) — `Promise.all`'a çevrildi. Dashboard istatistik ucu (`GET /workspaces/:id/statistics`, `get_workspace_statistics` RPC'si — kullanıcıya göre değil workspace'e göre tek sonuç) hiç cache'lenmiyordu; `projects` listesindeki 60sn'lik cache deseniyle aynı şekilde 30sn Redis cache eklendi (üye listesi ucuna BİLİNÇLİ OLARAK cache eklenmedi — 17 Ağustos'ta rol değişikliğinin görünmemesi bug'ı bu tür bir staleness'tan kaynaklanmıştı, aynı riski üye listesinde tekrar almamak için).

**Mobil (Flutter) eşitleme (madde 4, 5, 6):**
- Web'deki `AssigneesField` tasarımı birebir Flutter'a taşındı: yeni `mobile/lib/features/tasks/presentation/assignees_field.dart`, çekirdek seçim mantığı (`toggleAssigneeSelection`) saf bir fonksiyon olarak dışa çıkarılıp 7 birim testle kaplandı (`mobile/test/assignees_field_test.dart`). Eski `AssigneePickerField` (tekli) ve `ExtraAssigneesField` (çoklu) widget'ları tamamen silindi. Görev oluşturma (`create_task_dialog.dart`), düzenleme (`edit_task_dialog.dart` — **önceden burada çoklu atama düzenleme özelliği hiç yoktu, gerçek bir eksiklikti, artık var**) ve görev detayında (`task_detail_sheet.dart`) aynı bileşen kullanılıyor.
- Yan bulgu (kapsam dışı ama AssigneesField'ı bozduğu için düzeltildi): `workspace_members_provider.dart`'ta admin olmayan kullanıcılar için istemci-tarafı bir "yalnızca kendini gör" filtresi vardı. Backend `listMembers` (RLS-scoped `createUserClient` + `fix_workspace_members_select_all.sql`, 15 Ağustos) zaten HERKESE tüm üyeleri döndürüyor (web'deki "üyeler sayfası herkese açık" ile parity) — filtrenin kendisi güncel backend davranışıyla çelişen eski bir kalıntıydı, hem üye sayısını hem admin-olmayan kullanıcının AssigneesField'da başka birini seçebilmesini (web'deki gibi) engelliyordu. Kaldırıldı; backend'deki eski/yanlış docstring de düzeltildi.
- `dashboard_provider.dart`: statistics+tasks için sıralı iki Dio çağrısı artık `Future.wait` ile paralel (Dart 3 record `.wait` sözdizimi), her çağrının kendi hata yakalaması korundu.

**Test/doğrulama:**

- Backend: `npm run build` + `npm test` → **9/9 suite, 37/37 test** (RLS düzeltmelerinden sonra da değişmedi — bu oturumda yeni birim testi eklenmedi, canlı doğrulama aşağıdaki gerçek prod istekleriyle yapıldı).
- Frontend: `npx tsc --noEmit` temiz, `npm run build` başarılı (yalnızca önceden var olan, `cookies` kullanımından kaynaklanan "Dynamic server usage" bilgi mesajları — regresyon değil), `npx vitest run` → **3 dosya, 38/38 test** (değişmedi).
- Mobil: `flutter analyze` → **0 sorun** (paralel çalışan ajanın bıraktığı, kullanılmayan bir `submitting` state'inden kaynaklanan 2 dead-code uyarısı elle temizlendi), `flutter test` → **14/14** (7 → 14; yeni `assignees_field_test.dart`).
- **Canlı doğrulama (prod `staj-projesi-api.onrender.com`, push+deploy sonrası, gerçek/throwaway test hesaplarıyla — Node script'iyle uçtan uca):**
  - **Çoklu atama (madde 5):** register → login → workspace oluştur → görev oluştur → `PUT .../assignees` ile 2 gerçek kullanıcıya ata → `GET .../assignees` **2 kişiyi de döndürdü** (düzeltmeden önce bu adım boş dönüyordu, madde 5'in tam da bahsettiği belirti). Negatif testler: geçersiz payload (`user_ids` dizi değil) → temiz **400** (önceden validasyon yoktu); dizi içinde UUID olmayan bir değer → temiz **400**.
  - **Bildirim akışı (madde 2c — yukarıdaki ek bulgu):** davet gönder → `NotificationService.create()` RLS hatası vermeden **201** döndü → `GET /workspaces/:id/notifications` bildirimi **listede gösterdi** (düzeltmeden önce bu ya RLS hatası ya sessiz boş liste veriyordu) → `PATCH .../read` bildirimi **`is_read:true` yaptı**.
  - Bu iki canlı test, aynı zamanda `task.service.ts` ve `notification.service.ts`'teki `getAdminClient() ?? getClient()` düzeltmelerinin prod'da gerçekten devrede olduğunu da kanıtlıyor (yalnızca `npm run build`/`npm test` bunu kanıtlayamazdı — RLS hataları yalnızca gerçek bir Supabase bağlantısıyla ortaya çıkıyor).
- **Bilinen sınırlama (önceki oturumlarla aynı):** Bu ortamda (Windows, `chromium-cli` yok, Playwright kurulu değil) gerçek bir tarayıcı sürme aracı yok. Madde 1 (komut paleti), 2'nin web-tarafı JWT-yenileme kısmı ve 3 (presence paneli) gibi saf istemci-JS/UI davranışları kod incelemesi + `tsc`/`build`/test ile doğrulandı ama gerçek tıklama/Realtime akışıyla uçtan uca **test EDİLEMEDİ**. Web'deki çoklu atama düzeltmesi (embed kaldırma) bir Next.js server action'ı olduğu için doğrudan `curl` ile tetiklenemedi — ama NestJS/mobil ucu üzerinden yapılan canlı test, aynı `task_assignees` tablosunun/RLS'inin artık DB seviyesinde sorunsuz çalıştığını kanıtlıyor, bu da web tarafındaki düzeltmenin doğruluğuna dolaylı ama güçlü bir kanıt. Kullanıcının tarayıcıda elle doğrulaması öneriliyor.

**Yeni dosyalar:** `frontend/src/lib/command-actions.ts`, `frontend/src/lib/auth-token-refresh.ts`, `frontend/src/components/auth-token-refresh-provider.tsx`, `frontend/src/components/analytics/online-members-popover.tsx`, `frontend/src/components/task/assignees-field.tsx`, `backend/src/task/dto/set-task-assignees.dto.ts`, `mobile/lib/features/tasks/presentation/assignees_field.dart`, `mobile/test/assignees_field_test.dart`.
**Silinen dosyalar:** `mobile/lib/features/workspace/presentation/assignee_picker_field.dart`, `mobile/lib/features/workspace/presentation/extra_assignees_field.dart`.


### [22 Ağustos 2026] - Çoklu görev atamasının geri alınması + Mobil parite tamamlama (kullanıcı talebiyle)

> Kullanıcı, bir önceki paketten (bu sayfada hemen yukarıdaki giriş) hemen sonra iki talep bildirdi: (1) az önce eklenen çoklu görev ataması özelliğini TAMAMEN geri al, göreve eski tekli atama modeline dön; (2) bir önceki oturumda web'de yapılan 4 düzeltmenin (komut/eylem araması, bildirim canlı güncelleme, presence paneli, performans) mobilde de tek tek yapılıp yapılmadığını denetle, eksik olanları tamamla.

**1. Çoklu görev ataması TAMAMEN geri alındı (web + mobil + backend).**

- **Web:** `CreateTaskModal.tsx` ve `task-detail-sheet.tsx`'teki tek `AssigneesField` (sıralı çoklu seçim) bileşeni kaldırıldı, yerine eski tekli `<select id="task-assignee">` dropdown'ı geri getirildi (`assigneeId: string` state'i). `frontend/src/app/actions/task-assignees.ts` (getTaskAssignees/setTaskAssignees) ve `frontend/src/components/task/assignees-field.tsx` **silindi**. `create-task.ts`'ten `extraAssigneeIds` parametresi ve ilgili `setTaskAssignees` çağrısı kaldırıldı. Artık kullanılmayan i18n anahtarları (`taskModal.assignees`, `assigneesHint`, `extraAssignees`, `extraAssigneesHint`, `primary`, `noMembers`) tr.json/en.json'dan temizlendi.
- **Backend:** `PUT/GET /workspaces/:id/tasks/:id/assignees` uçları (`task.controller.ts`) ve `TaskService.listAssignees`/`setAssignees`/`isMissingAssigneesTable` metodları (`task.service.ts`) kaldırıldı. `backend/src/task/dto/set-task-assignees.dto.ts` **silindi**.
- **Mobil:** Web'deki `AssigneesField`'in mobil karşılığı (`assignees_field.dart`), `task_assignees_provider.dart` ve `mobile/test/assignees_field_test.dart` **silindi**. Eski tekli `AssigneePickerField` (git geçmişinden aynen geri getirildi) `create_task_dialog.dart` ve `edit_task_dialog.dart`'a yeniden bağlandı. `task_detail_sheet.dart`'taki `_AssigneesSection`/`_AssigneeChip` (çoklu atama gösterimi + düzenleme diyaloğu) kaldırılıp eski salt-okunur `Text(assigneeLabel)` gösterimine dönüldü. `task_repository.dart`'tan `getAssignees`/`setAssignees`/`TaskAssigneeOption`, `task_provider.dart`'tan `extraAssigneeIds` parametresi, `api_constants.dart`'tan `workspaceTaskAssignees` yolu kaldırıldı.
- **Veri tarafı — BİLİNÇLİ OLARAK OTOMATİK ÇALIŞTIRILMADI:** Yeni `database/migrations/revert_task_assignees_to_single.sql` — bu projenin tüm şema/veri değişikliklerinde olduğu gibi kullanıcının Supabase SQL Editor'de **elle** çalıştırması gereken bir migration dosyası. İçeriği: (ADIM 0) hiçbir şeyi değiştirmeyen, yalnızca "kaç kayıt etkilenecek" sorusunu cevaplayan bir SELECT; (ADIM 1, opsiyonel) silinecek satırların bir yedek tabloya (`_task_assignees_backup_20260822`) kopyalanması; (ADIM 2) görevin birincil atananıyla (assignee_id/assigned_to) eşleşmeyen `task_assignees` satırlarının silinmesi — **geri dönüşü yok**, bu yüzden dosyada büyük harflerle uyarılmış ve ADIM 0/1'den sonra çalıştırılması istenmiş; (ADIM 3) doğrulama. `task_assignees` tablosunun kendisi ve ilgili RLS politikaları/`SECURITY DEFINER` fonksiyonları (`is_task_assignee`, `can_view_task`) BİLİNÇLİ OLARAK DROP EDİLMEDİ — gerekçe dosyanın başında ayrıntılı yazılı (kısaca: uygulama artık bu tabloya hiç yazmadığından bu fonksiyonlar veri temizliğinden sonra zaten hep FALSE dönüp etkisiz kalacak; DROP etmek önceki bir recursion bug'ını (`fix_tasks_task_assignees_rls_recursion.sql`) yanlış geri alma riski taşırdı, hiçbir fonksiyonel fayda karşılığında). **Bu oturumda gerçek üretim verisindeki risk miktarı (kaç görevin fazladan ataması olduğu) ÖLÇÜLEMEDİ** — yerelde `SUPABASE_SERVICE_ROLE_KEY` yok, doğrudan DB sorgusu atılamadı; kullanıcının migration'ın ADIM 0'ını çalıştırıp sonucu görmesi ve ADIM 2'yi yalnızca o sayıyı gördükten sonra çalıştırması öneriliyor.

**2. Mobil parite denetimi — 4 madde tek tek kontrol edildi, 3'ü eksik bulunup tamamlandı.**

- **(a) Komut/eylem araması — EKSİKTİ, tamamlandı.** `global_search_screen.dart` yalnızca içerik (proje/görev/üye/not) arıyordu, web'deki gibi bir "eylem tetikle" kavramı yoktu. Yeni bir "KOMUTLAR" bölümü eklendi: Üye Davet Et (`showInviteMemberDialog`), Yeni Workspace Oluştur (`workspace_switcher.dart`'taki özel `_showCreateWorkspaceDialog` **public** yapılıp `showCreateWorkspaceDialog` olarak yeniden adlandırıldı, hem switcher hem arama ekranından çağrılabiliyor), Temayı Değiştir (`themeModeProvider.setThemeMode`). "Yeni Görev Oluştur" web'deki gibi eklenmedi — mobil aramanın bir proje bağlamı yok (web'de yalnızca proje sayfasındayken görünüyordu), mobilde zaten her proje ekranında bir "+" FAB'ı ile bire-bir aynı işlev doğrudan erişilebilir; aramaya eklemek gereksiz bir dolambaç olurdu.
- **(b) Bildirim canlı güncelleme — kısmen eksikti, tamamlandı.** Yeni bildirim geldiğinde güncelleme zaten doğru bağlıydı (`realtime_provider.dart`'ın `onNewNotification` handler'ı `notificationsProvider`'ı invalidate ediyordu) — ayrıca bu oturumun bir önceki paketindeki backend RLS düzeltmesi (`notification.service.ts`) sayesinde bu event artık gerçekten tetikleniyor. AMA "okundu" tarafı eksikti: backend `markAsRead`/`markAllAsRead` `notification_read`/`notifications_read_all` WebSocket event'leri emit ediyordu ama mobil `socket_service.dart` bu event'leri HİÇ dinlemiyordu — yani bir bildirim web'de (veya başka bir cihazda) okundu işaretlenirse, mobil açık kalan oturumda sayaç/liste canlı güncellenmiyordu, yalnızca bir sonraki manuel yenilemede düzeliyordu. `socket_service.dart`'a yeni `onNotificationRead` callback'i + `realtime_provider.dart`'a bağlantısı eklendi (`notificationsProvider`'ı invalidate ediyor).
- **(c) Presence paneli — EKSİKTİ, tamamlandı.** Dashboard'daki "Aktif Üyeler" KPI kartı düz bir sayıydı, dokunulamıyordu. Artık dokununca bir bottom sheet açılıyor (`_showOnlineMembersSheet`, `dashboard_screen.dart`), `workspacePresenceProvider`'daki online ID'leri `workspaceMembersProvider`'daki ad/baş harf avatarıyla eşleştirip listeliyor — web'deki `online-members-popover.tsx`'in birebir mobil karşılığı.
- **(d) Performans — bir eksik daha bulundu, tamamlandı.** `dashboard_provider.dart`'taki paralelleştirme zaten bu oturumun bir önceki paketinde yapılmıştı. Ayrı bir araştırma turunda (tüm provider dosyaları tek tek tarandı) `project_provider.dart`'ın `ProjectsNotifier._applyVisibilityFilter` (hem `build()` hem `refresh()` tarafından çağrılıyor) içinde AYNI sınıf bug bulundu: projeler tam çekilmeden görev-görünürlüğü sorgusu (Member/Guest için) hiç başlamıyordu, halbuki ikisi de yalnızca workspaceId/userId'ye bağlı, birbirinin sonucuna değil. `isAdmin` zaten senkron/yerel olarak hesaplanabildiğinden, iki sorgu artık Dart 3 `.wait` ile paralel başlatılıp öyle birleştiriliyor. Diğer tüm provider dosyaları (task/member/comment/subtask/file/admin/invitation/notification/personal/progress-report/activity-log/search/auth) tek tek tarandı, başka bir örnek bulunmadı — bu app'in mimarisi (bağımsız veri ihtiyacı = bağımsız provider, Flutter/Riverpod bunları zaten paralel çözüyor) bu sınıf hatayı doğal olarak büyük ölçüde önlüyor.

**Test/doğrulama:**

- Backend: `npm run build` + `npm test` → **9/9 suite, 37/37 test** (multi-assign uçları kaldırıldıktan sonra da değişmedi — hiçbir mevcut test bu uçlara özel değildi).
- Frontend: `npx tsc --noEmit` temiz, `npm run build` başarılı, `npx vitest run` → **3 dosya, 38/38 test** (değişmedi).
- Mobil: `flutter analyze` → **0 sorun**, `flutter test` → **7/7** (14 → 7; `assignees_field_test.dart` kendisiyle birlikte kaldırıldığı için beklenen düşüş, kalan 7 test — API path helper'ları + widget boot testi — hepsi geçiyor).
- **Canlı doğrulama (prod `staj-projesi-api.onrender.com`, push+deploy sonrası, gerçek/throwaway test hesaplarıyla — Node script'iyle uçtan uca):** register → login → workspace oluştur → `assignee_id` ile tek kişiye görev oluştur (**başarılı**, dönen görevde `assignee_id` doğru) → `PATCH .../tasks/:id` ile farklı bir tek kişiye yeniden ata (**başarılı**) → eski çoklu atama uçlarının artık gerçekten kaldırıldığını doğrula: `GET`/`PUT .../tasks/:id/assignees` ikisi de **404** döndü (route seviyesinde silindi, yalnızca "yetkisiz" değil "yok"). Bu, revert'in prod'da gerçekten devrede olduğunu kanıtlıyor.
- **Bilinen sınırlama:** Bu ortamda (Windows) `chromium-cli`/Playwright yok VE bu denetim sırasında **fiziksel bir Android cihaz da bağlı değildi** (`adb devices` boş, `flutter devices` yalnızca Windows desktop + Chrome/Edge web hedeflerini gösterdi). Kullanıcı "gerçek cihazda USB bağlı deneyerek doğrula" istedi ama bu oturumda hiçbir cihaz erişilebilir değildi — mobil taraftaki tüm değişiklikler yalnızca `flutter analyze`/`flutter test` + (backend uçları için) canlı REST istekleriyle doğrulanabildi, gerçek dokunma/UI akışıyla test EDİLEMEDİ. Kullanıcının bir cihaz bağlayıp elle doğrulaması önemle öneriliyor — özellikle madde (a)'nın yeni "Komutlar" bölümü, (c)'nin yeni bottom sheet'i ve tekli atama seçicisinin gerçek dokunmatik akışı.
...
### [23 Ağustos 2026] - Web + Backend + Mobil: canlı ölçümle performans profillemesi ve düzeltmeleri (kullanıcı talebiyle)

> Kullanıcı, varsayımla düzeltme yapılmamasını, önce gerçekten yavaş olan noktaların ÖLÇÜLEREK bulunmasını istedi. Bu oturumda tarayıcı/DevTools erişimi olmadığından, prod'a karşı gerçek throwaway test verisiyle (1 workspace + 1 proje + 14 görev + 2 kullanıcı) bir Node.js ölçüm harness'i kuruldu: backend uçları doğrudan HTTP ile, web sayfaları ise `sb_access_token`/`auth_session`/`active_workspace_id` cookie'leri elle set edilerek SSR yanıt süresi ölçülerek test edildi. Her ölçüm 5-6 tekrarla yapılıp medyan alındı (Render free-tier + internet gecikmesi tek ölçümü güvenilmez kılıyor).

**Bulunan gerçek darboğazlar (kod incelemesiyle DOĞRULANMIŞ, varsayım değil):**

1. **`WorkspaceService.search()` (backend)** — `auth.getUser()` çağrısı, workspace-scoped 4 sorgudan (projeler/görevler/üyeler/owner) ÖNCE, SIRALI atılıyordu (kişisel not/todo sorguları `userId`'ye bağımlı olduğu için). En yavaş backend ucuydu (1259ms medyan). Artık `auth.getUser()` diğer 4 sorguyla AYNI `Promise.all`'da paralel; yalnızca `userId`'ye bağımlı not/todo sorguları ikinci (kısa) bir paralel turda.
2. **`WorkspaceService.listMembers()` (backend)** — "tüm üyeler" sorgusu, owner/membership kontrolünden SONRA sıralı atılıyordu; üçü de birbirinden bağımsız, tek `Promise.all`'a taşındı (1084ms → 937ms medyan).
3. **`TaskService.findAll()` (backend, görev listesi)** — `WorkspaceRoleGuard`'ın istek başına zaten hesapladığı "admin mi" bilgisini YOK SAYIP `isWorkspaceAdmin()` ile AYNI sorguyu ikinci kez, sıralı olarak atıyordu. Guard artık `request.workspaceRole`'e hesapladığı rolü yazıyor (`isOwner`, `role`, `isAdmin`); controller bunu `@Req()` ile okuyup servise iletiyor, servis varsa tekrar sorgulamıyor (957ms → 747ms medyan).
4. **KRİTİK — 11 rotada `WorkspaceRoleGuard` hem class hem method seviyesinde `@UseGuards` ile İKİ KEZ çalıştırılıyordu** (`task.controller.ts`'te 7, `workspace.controller.ts`'te 2, `project.controller.ts`'te 2 rota) — her çalışma kendi 2 paralel Supabase sorgusunu atıyordu, yani bu yazma/silme/davet uçları auth kontrolünü gereksiz yere iki katına çıkarıyordu. Bu, NestJS'in class-level ve method-level guard'ları BİRİKTİRİP İKİSİNİ DE çalıştırması (deduplicate etmemesi) nedeniyle oluşan, kod tabanında yaygın bir "iki kez @UseGuards yazma" alışkanlığından kaynaklanıyordu. Redundant method-level `@UseGuards(SupabaseAuthGuard, WorkspaceRoleGuard)` satırları kaldırıldı (`@Roles(...)` metadata'sı korundu — o zaten Reflector ile okunuyor, hangi guard çağrısının okuduğu önemli değil).
5. **`/projects` sayfası (web SSR)** — `resolveWorkspaceRole`, projeler+istatistikleri taşıyan `Promise.all`'dan SONRA sıralı çalışıyordu. Ölçülen en yavaş SSR sayfasıydı (2445ms medyan). Artık aynı `Promise.all`'da paralel → **1244ms medyan (%49 daha hızlı)**.
6. **`/project/[id]` sayfası (web SSR)** — aynı sınıf sorun. `workspaceId` URL'den zaten biliniyorsa (uygulama içi linkler hep `withWorkspaceQuery` ile taşır — yaygın durum) artık `resolveWorkspaceRole` projeyle paralel; yalnızca nadir fallback durumunda (workspaceId bilinmiyor, `project.workspace_id`'ye ihtiyaç var) sıralı kalıyor. 1986ms → 1571ms medyan (%21 daha hızlı).

**Mobil:** `flutter run`/interaktif profil aracı gerektiren canlı widget-rebuild ölçümü bu oturumda yapılamadı (aşağıdaki sınırlamaya bkz.). Bunun yerine: (a) `kDebugMode`-gated bir Dio timing interceptor eklendi (`api_client.dart`) — release build'i etkilemez, gelecekteki debug oturumlarında her isteğin süresini loglar; (b) tüm Riverpod provider'ları `ref.watch(...).select(...)` kullanımı açısından grep ile tarandı — `workspaceProvider` için select'siz hiçbir whole-object watch bulunamadı, kod tabanı zaten tutarlı; (c) `dashboard_provider.dart` (statistics+tasks) ve `project_provider.dart` (projeler+görev-görünürlüğü) paralelleştirmeleri ÖNCEKİ oturumlarda zaten yapılmıştı, bu oturumda yeni bir mobil-taraf waterfall bulunmadı.

**Operasyonel bulgu — Render free-tier bağlantı sıkışması (gerçek bir kod bug'ı DEĞİL):** Ölçüm sırasında `TaskService.findAll`/`findOne` bir turda geçici olarak boş/404 döndü (14 görevlik bir workspace'te 0 görev, var olan bir görev "bulunamadı"); `statistics`/`search` uçları AYNI ANDA doğru veri döndürmeye devam etti. İlk şüphe kod regresyonuydu (`findAll`'daki `precomputedIsAdmin` değişikliği), ama `findOne` (bu oturumda hiç değiştirilmedi) da aynı anda etkilendiği ve birkaç dakika sonra HER İKİSİ de sorunsuz çalıştığı için bu, art arda çok sayıda test isteği göndermenin (bu oturumda kısa sürede onlarca istek atıldı) Render/Supabase free-tier bağlantı havuzunu geçici olarak sıkıştırmasıyla açıklanıyor — kod değişikliği değil. Not olarak düşüldü: gelecekte "üretimde ara sıra 404/boş liste" şikâyeti gelirse, önce free-tier bağlantı limitlerini/yoğunluğunu düşünün, hemen kod regresyonu varsaymayın.

**Test/doğrulama:**

- Backend: `npm run build` + `npm test` → **9/9 suite, 37/37 test**.
- Frontend: `npx tsc --noEmit` temiz, `npm run build` başarılı, `npx vitest run` → **3 dosya, 38/38 test**.
- Mobil: `flutter analyze` → **0 sorun**, `flutter test` → **7/7**. Release APK yeniden derlenip (`flutter build apk --release --dart-define=API_BASE_URL=...`) fiziksel cihaza (2201116TG) tekrar kuruldu — kurulum başarılı, bu bugünkü koddan (Dio interceptor dahil) hiçbir derleme/kurulum regresyonu olmadığını kanıtlıyor. **Bilinen sınırlama:** bu oturumda da dokunmatik/UI otomasyon aracı yoktu — cihazda önceki oturumlarda düzeltilen parite maddelerinin (arama komutları, presence paneli, tekli atama, bildirim okundu senkronu) gerçek dokunmatik akışla YENİDEN test edilmesi, yalnızca kodun değişmediği (bu oturum mobilde yalnızca debug-only bir logger ekledi, davranışsal hiçbir değişiklik yapmadı) gerekçesiyle atlandı — kullanıcının cihazda elle doğrulaması önemle öneriliyor.
- **Canlı önce/sonra karşılaştırması (prod, push+deploy sonrası, aynı sabit test workspace'i — 1 proje + 14 görev):**

| Uç/Sayfa | Önce (medyan) | Sonra (medyan) | Fark |
|---|---|---|---|
| Backend `GET .../tasks` | 957ms | 747ms | %22 daha hızlı |
| Backend `GET .../members` | 1084ms | 937ms | %14 daha hızlı |
| Backend `GET .../search?q=` | 1259ms | 1228ms | ~aynı (gürültü payı içinde) |
| Web SSR `/projects` | 2445ms | 1244ms | **%49 daha hızlı** |
| Web SSR `/project/[id]` | 1986ms | 1571ms | %21 daha hızlı |
| Web SSR `/` (dashboard) | 2194ms | 1749ms | %20 daha hızlı (dolaylı — bu sayfanın kodu değişmedi, muhtemelen ölçüm gürültüsü/ısınmış cache) |
| Web SSR `/members` | 996ms | 957ms | ~aynı (bu sayfa backend'i değil doğrudan Supabase'i kullanıyor, düzeltmeden etkilenmedi — beklenen) |

**Dürüst değerlendirme:** Render free-tier + gerçek internet üzerinden ölçüm doğası gereği gürültülü (tek istek 500-1800ms aralığında ±%30-50 sallanabiliyor). `/projects` ve `/project/[id]`'deki iyileşme, tam olarak kod değişikliği yapılan yerlerle örtüştüğü ve tutarlı büyüklükte olduğu için güvenilir kabul ediliyor. Diğer bazı satırlardaki (`/`, `/members`) fark muhtemelen ölçüm gürültüsü — o sayfaların kodu bu oturumda değişmedi. En sağlam iddia, sayı bazlı iyileşmeden çok **yapısal** olan: kaç round-trip'in gerçekten elendiği (kod diff'iyle kanıtlanabilir), somut ms rakamları ise destekleyici ama gürültülü bir kanıt.

**Yeni dosyalar/değişenler:** `backend/src/auth/guards/workspace-role.guard.ts`, `backend/src/task/task.controller.ts`, `backend/src/task/task.service.ts`, `backend/src/workspace/workspace.controller.ts`, `backend/src/workspace/workspace.service.ts`, `backend/src/project/project.controller.ts`, `frontend/src/app/(dashboard)/projects/page.tsx`, `frontend/src/app/(dashboard)/project/[id]/page.tsx`, `mobile/lib/core/network/api_client.dart`.


### [8 Eylül 2026] - Login OTP (e-posta ile giriş onayı) ve gerçek e-posta zorunluluğu (kullanıcı talebiyle)

> Kullanıcı, girişte e-posta tabanlı iki adımlı doğrulama (TOTP'a ek/alternatif olarak) ve kayıtta gerçek e-posta zorunluluğu istedi. Bu oturuma başlarken backend tarafı (`auth.service.ts`, `auth.controller.ts`, `mail.service.ts`, `login-otp.dto.ts`, `auth.service.spec.ts`) **daha önceki, yarım kalmış bir oturumdan zaten commit'lenmemiş halde kod tabanında hazır bulundu** — bu oturum önce bunu inceleyip `npm run build`+`npm test` ile doğruladı, sonra web ve mobil entegrasyonunu sıfırdan ekledi.

**1. Backend (zaten hazır bulundu, bu oturumda yalnızca doğrulandı) — `backend/src/auth/`:**
- `register()`: `mailService.enabled` (SMTP tanımlıysa) `admin.auth.admin.generateLink({type:'signup', ...})` kullanıyor — kullanıcı ONAYSIZ oluşturulup link kendi `MailService`'imizle gönderiliyor (Supabase'in kendi mailer rate limitine takılmadan). SMTP tanımlı DEĞİLSE bilinçli olarak eski otomatik-onaylı davranışa (`email_confirm: true`) düşüyor — SMTP yoksa link hiç teslim edilemeyeceği için hesap sonsuza dek kilitli kalmasın diye. **ÖNEMLİ — elle yapılması gereken adım:** Bu kod tek başına yeterli değil, Supabase Dashboard → Authentication → Providers → Email → **"Confirm email" seçeneğinin de AÇIK olması gerekiyor** (bu bir dashboard ayarı, koddan değiştirilemez) — açık değilse Supabase kullanıcıyı `generateLink` sonrası da otomatik onaylı sayabilir. Kullanıcının bunu kontrol etmesi gerekiyor, bu oturumda doğrulanamadı.
- `login()`: Supabase `"Email not confirmed"` hatası artık Türkçe'ye çevriliyor ("Lütfen önce e-postanıza gelen onay linkine tıklayın."). Şifre doğrulandıktan sonra `hasVerifiedTotp()` ile hesapta doğrulanmış bir TOTP faktörü olup olmadığına bakılıyor: varsa mevcut davranış (token doğrudan dönüyor, web/mobil kendi AAL1→AAL2 MFA akışını başlatıyor) **hiç değişmedi**; yoksa yeni `issueLoginOtp()` çağrılıp `{otp_required: true, user_id, message}` dönüyor (token DÖNMÜYOR).
- `issueLoginOtp()`: 6 haneli kod (`crypto.randomInt`) üretip Redis'e (`@nestjs/cache-manager`, zaten global) `login_otp:{userId}` anahtarıyla **5 dakika TTL** ile yazıyor; aynı zamanda `login_otp_cooldown:{userId}` ile **60 saniyelik** bir cooldown yazıyor (spam/brute-force koruması — cooldown içinde tekrar istenirse 429 `HttpException` ile "N saniye sonra tekrar deneyin" dönüyor). `MailService.sendLoginOtpEmail()` ile kod e-postalanıyor (konu: "Giriş Onay Kodu", içerik: kod + "5 dakika geçerlidir, siz yapmadıysanız yok sayın").
- Yeni uçlar: `POST /auth/login/request-otp` (`requestLoginOtp` — şifreyi yeniden doğrulayıp aynı `issueLoginOtp()`'u çağırır, TOTP aktifse 400 döner; "kodu tekrar gönder" için kullanılır) ve `POST /auth/login/verify-otp` (`verifyLoginOtp` — Redis'ten kodu okur, eşleşiyorsa siler ve `login()`'de üretilmiş session'ı aynen döner, eşleşmiyorsa/süresi dolmuşsa 401). İkisi de mevcut Swagger deseniyle belgeli, mevcut `AUTH_THROTTLE` (5 istek/dk/IP) ile sınırlı.
- `login-otp.dto.ts`: `RequestLoginOtpDto` (email + password, `class-validator`), `VerifyLoginOtpDto` (user_id + 6 haneli code).
- `auth.service.spec.ts`: 10 yeni birim testi (`SupabaseService`/`MailService`/`CACHE_MANAGER` mock'lanmış) — TOTP varsa OTP'ye hiç girmediğini, TOTP yoksa token DÖNMEYİP OTP başlattığını, "Email not confirmed"'in Türkçe'ye çevrildiğini, cooldown'ın 429 verdiğini, doğru/yanlış/süresi-dolmuş kod senaryolarını kapsıyor.

**2. Web (`frontend/src/`) — bu oturumda sıfırdan eklendi:**
- Yeni `components/auth/email-otp-challenge-card.tsx` (`EmailOtpChallengeCard`) — mevcut `MfaChallengeCard` ile aynı görsel/UX deseni: 6 haneli kod girişi, doğrula butonu, **60 saniyelik geri sayımlı "Kodu tekrar gönder"** butonu (backend zaten rate-limitli, UI ayrıca kendi sayacını tutuyor), iptal butonu. Props: `email`/`password` (resend için — `/auth/login/request-otp` şifreyi yeniden istiyor), `userId`, `onVerified`, `onCancel`.
- `app/(auth)/login/page.tsx`: `onSubmit` artık `/auth/login` cevabında `otp_required`/`user_id` var mı diye bakıyor; varsa `otpPending` state'ine geçip `EmailOtpChallengeCard`'ı gösteriyor (üçüncü dal — mevcut `mfaPending` dalının yanına eklendi, dokunulmadı). MFA ve OTP sonrası ortak "oturumu kalıcı yaz + tekrar `needsMfaChallenge()` kontrol et + yönlendir" mantığı `afterAuthenticated()` adında paylaşılan bir fonksiyona çıkarıldı (önceden yalnızca normal login yolunda vardı, kopyalanmadı).
- `i18n/dictionaries/{tr,en}.json`: `auth.otp*` anahtarları eklendi (title/subtitle/code/verify/cancel/success/fail/resend/resendCountdown/resendSuccess/resendFail) — mobildeki karşılıklarıyla birebir aynı metin.
- Register sayfası (`register/page.tsx`) **hiç değiştirilmedi** — zaten kayıt sonrası otomatik oturum almak için dahili bir `/auth/login` çağrısı yapıyor ve `if (!data.access_token)` durumunda "Kayıt başarılı, giriş yapabilirsiniz" diyip `/login`'e yönlendiriyordu. Yeni backend davranışıyla (taze kayıtlı kullanıcıda TOTP olamayacağı için bu dahili login artık `otp_required` dönüyor) bu mevcut fallback DOĞRU ÇALIŞIYOR — kullanıcı otomatik login olmak yerine `/login`'e yönlenip orada (artık var olan) OTP akışını görüyor. Kasıtlı bir tasarım değişikliği değil, mevcut kodun yeni davranışla kazara-doğru uyumu; not olarak düşüldü.

**3. Mobil (Flutter, `mobile/lib/`) — bu oturumda sıfırdan eklendi:**
- `features/auth/providers/auth_provider.dart`: Yeni `AuthStatus.otpPending` + `AuthState.otpPending({otpUserId, otpEmail, otpPassword})` (TOTP'un `mfaPending`iyle aynı desen, birbirini dışlıyor). `login()` artık `repository.login()`'den `LoginResult` alıyor (`session` ya da `otpUserId`) ve `otp_required` durumunda `otpPending` state'ine geçiyor. Yeni `submitEmailOtpCode()`, `resendEmailOtp()`, `cancelEmailOtpChallenge()` metodları `submitMfaCode`/`cancelMfaChallenge` ile birebir aynı desende.
- `features/auth/data/auth_repository.dart`: Yeni `LoginResult` (session/otpRequired) ve `RegisterResult` (session/needsManualLogin) tipleri. `login()` artık `AuthSession` değil `LoginResult` dönüyor (`otp_required`/`user_id` alanlarını okuyan `_loginResultFromResponse`/`_tryReadOtpUserId` ile). Yeni `requestLoginOtp()`/`verifyLoginOtp()` metodları eklendi (`ApiConstants.authLoginRequestOtp`/`authLoginVerifyOtp` — `/auth/login/request-otp`, `/auth/login/verify-otp`).
- `features/auth/presentation/login_screen.dart`: Yeni `_EmailOtpChallengeCard` (mevcut `_MfaChallengeCard` ile aynı desen + 60 saniyelik `Timer.periodic` geri sayımlı "kodu tekrar gönder"), `LoginScreen.build()`'e üçüncü dal (`AuthStatus.otpPending`) eklendi.
- `core/l10n/app_strings.dart`: `auth.otp*` getter'ları + TR/EN sözlük girişleri (web'dekiyle birebir aynı metin), ayrıca `auth.registerSuccessLogin` (aşağıya bkz.).
- **register() imzası değişti — `Future<bool>` → `Future<RegisterOutcome>` (authenticated/needsManualLogin/failed).** Web'deki gibi: `AuthRepository.register()`, `/auth/register` sonrası token almak için dahili `login()` çağırıyordu; artık bu dahili çağrı `otp_required` dönebildiği için (taze kayıtlı kullanıcıda TOTP olamaz) `register()`'ın "her zaman ya oturum alır ya da hata fırlatır" varsayımı artık GEÇERSİZ — üçüncü, hatasız ama oturumsuz bir sonuç (`needsManualLogin`) eklendi. `RegisterScreen._submit()` bu üç durumu ayrı ayrı ele alıyor: `needsManualLogin` durumunda web'deki "Kayıt başarılı, giriş yapabilirsiniz" mesajını gösterip `/login`'e yönlendiriyor (yeni `authRegisterSuccessLogin` string'i).
- **BUG (bu oturumda bulundu ve düzeltildi, TOTP akışını da etkiliyordu) — `core/router/app_router.dart`'ın `redirect` mantığı, `mfaPending`/`otpPending` state'lerini "authenticated ama token henüz yok (geçiş halinde)" ile karıştırıyordu.** Her iki state de `token=null` bıraktığından, mevcut `if (auth.token == null) return isSplash ? null : AppRoutes.splash;` kuralı, kullanıcı `/login`'deyken (challenge kartını görmesi gerekirken) onu `/splash`'a yönlendiriyor, orada da aynı kural `isSplash=true` olduğu için `null` (kal) döndüğünden **kullanıcı splash ekranında sonsuza dek takılı kalabiliyordu** — challenge kartı hiç render edilmeden. Bu, TOTP MFA akışını da (bu oturumdan önce de var olan bir state) etkileyen, önceden fark edilmemiş bir bug'dı (CLAUDE.md Bölüm 7'de not edildiği gibi mobilde daha önce yalnızca kod incelemesiyle doğrulanmıştı, interaktif dokunma testi hiç yapılmamıştı). Düzeltme: `token == null` kontrolünden ÖNCE, `mfaPending`/`otpPending` için `isAuthRoute ? null : AppRoutes.login` döndüren açık bir guard eklendi — böylece hem yeni email-OTP akışı hem de mevcut TOTP akışı `/login`'de kalıp kendi challenge kartını gösterebiliyor.
- **Küçük UX düzeltmesi (aynı kök sebeple ilişkili):** `LoginScreen._submit()`, `login()` `false` döndüğünde (ki bu hem gerçek hata hem de `mfaPending`/`otpPending`'e geçiş için olabiliyordu) HER ZAMAN "giriş başarısız" toast'ı gösteriyordu — `errorMessage` boşsa `s.authLoginFailed`'e düşerek. Artık yalnızca durum gerçekten `AuthStatus.unauthenticated` ise toast gösteriliyor; MFA/OTP'ye geçişte yanlış "başarısız" toast'ı bir daha çıkmıyor.

**Test/doğrulama:**

- Backend: `npm run build` + `npm test` → **10/10 suite, 47/47 test** (9/30 → 10/47; yeni `auth.service.spec.ts`'in 10 testi dahil).
- Frontend: `npx tsc --noEmit` temiz, `npm run build` başarılı (yalnızca önceden var olan `cookies`/"Dynamic server usage" bilgi mesajları — regresyon değil), `npx vitest run` → **3 dosya, 38/38 test** (değişmedi, bu oturumda yeni bir frontend unit test eklenmedi — yalnızca UI bileşeni, mevcut test dosyaları bu alanı kapsamıyordu).
- Mobil: `flutter analyze` → **0 sorun** (2 `prefer_initializing_formals` uyarısı elle temizlendi), `flutter test` → **7/7** (değişmedi).
- **Bilinen sınırlama — gerçek uçtan uca (register → login → Redis'te kod → verify) test EDİLEMEDİ:** Bu ortamda yerel `backend/.env` yok (gerçek Supabase anahtarları yok), Redis/redis-cli/Docker da kurulu değil — yerel bir backend'i gerçek Supabase'e karşı çalıştırıp uçtan uca deneyecek kimlik bilgisi bu oturumda mevcut değildi. Prod'a karşı (push+deploy sonrası) da tam mutlu-yol testi (gerçek e-postaya gelen kodu okuyup girme) yapılamaz — bu makineden prod SMTP/Redis'e/gerçek bir e-posta kutusuna erişim yok. Kullanıcının prod'da gerçek bir hesapla deneyip (1) kayıt sonrası "Kayıt başarılı, giriş yapabilirsiniz" mesajını görmesi, (2) girişte e-posta OTP ekranının açıldığını, (3) e-postaya kodun geldiğini (SMTP_HOST Render'da tanımlıysa) ve (4) doğru kodla girişin tamamlandığını elle doğrulaması öneriliyor. Ayrıca Supabase Dashboard'daki "Confirm email" ayarının açık olduğu da bu oturumda doğrulanamadı (madde 1'e bkz.) — kullanıcının kontrol etmesi gerekiyor.

**Yeni dosyalar:** `frontend/src/components/auth/email-otp-challenge-card.tsx` (bu oturumda), `backend/src/auth/dto/login-otp.dto.ts` + `backend/src/auth/auth.service.spec.ts` (önceki yarım kalmış oturumdan, bu oturumda yalnızca doğrulandı).


### [8 Eylül 2026] - Login OTP'nin SMTP altyapı sorunu nedeniyle TAMAMEN geri alınması (aynı gün, kullanıcı talebiyle)

> Yukarıdaki paket deploy edildikten hemen sonra, kullanıcı gerçek hesabıyla canlıda denedi: kayıt oluyor ama onay/OTP e-postaları hiç gelmiyordu. Kök nedeni bulmak için birkaç tur canlı teşhis yapıldı, sonunda kullanıcı TÜM iki adımlı doğrulamayı (bugünkü e-posta OTP'si VE önceki bir oturumdan gelen TOTP/Authenticator MFA'sı) geri almaya karar verdi. Bu bölüm hem teşhis sürecini hem geri alma işleminin kapsamını kayda geçiriyor.

**Teşhis süreci (üç ayrı düzeltme denendi, sonuncusu asıl kök nedeni ortaya çıkardı):**

1. **İlk hata:** Kullanıcı Render'a `SMTP_HOST=smtp.gmail.com` + Gmail App Password girdi. İlk canlı denemede kayıt isteği 30sn'de zaman aşımına uğradı, Render loglarında `connect ENETUNREACH 2a00:1450:...:587` görüldü — IPv6 adresi. Kök neden: nodemailer, `smtp.gmail.com` için hem A hem AAAA kaydını çözüp aralarından RASTGELE seçiyor (`node_modules/nodemailer/lib/shared/index.js` okunarak doğrulandı); Render konteynerlerinde gerçek IPv6 çıkışı yok. nodemailer'da bunu kapatacak bir `family` seçeneği YOK (koddan kontrol edildi, `family: 4` geçmek hiçbir şey yapmıyor). **Düzeltme:** `mail.service.ts`, `dns.lookup(host, {family:4})` ile IPv4 adresini kendi çözüp nodemailer'a literal IP + `tls.servername` veriyordu (yerel Gmail testinde TLS bağlantısı kuruldu, yalnızca sahte parola ile AUTH reddedildi — mekanizma çalışıyordu).
2. **İkinci deneme hâlâ başarısız:** Aynı düzeltme deploy edildikten sonra kayıt yine zaman aşımına uğradı, ama bu sefer hata **"Connection timeout"**a dönüştü (ENETUNREACH değil) — yani artık doğru (IPv4) adrese gidiyor ama bağlantı hiç kurulamıyor.
3. **Kesin teşhis:** Render'ın ücretsiz planında Shell erişimi olmadığı için (yükseltme istiyor), backend'e geçici bir `GET /health/net-check` ucu eklendi — konteynerin içinden `smtp.gmail.com:587`, `:465` ve kontrol için `google.com:443`'e ham `net.connect` denemesi yapıp sonucu JSON döndürüyordu. Canlı sonuç: **587 ve 465 ikisi de ~260ms'de REDDEDİLDİ, 443 17ms'de başarılı.** Bu, Render'ın ücretsiz planının giden SMTP bağlantılarını (spam önleme amaçlı, birçok PaaS'ta standart bir kısıtlama) TAMAMEN engellediğini kanıtladı — kod düzeltmesiyle çözülemeyecek bir altyapı kısıtlaması.

**Bu noktada önerilen ama kullanıcı tarafından reddedilen çözüm:** SMTP yerine bir e-posta sağlayıcısının HTTP API'sine (örn. Resend — 443 üzerinden çalıştığı için hiç engellenmiyor) geçmek. Kullanıcı bunun yerine **iki adımlı doğrulamanın tamamının (bugünkü VE önceki günlerdeki) kaldırılmasını** istedi.

**Geri alınanlar (tam liste):**

- **Backend:** `POST /auth/login/request-otp`, `POST /auth/login/verify-otp`, `POST /auth/mfa/status`, `POST /auth/mfa/challenge`, `POST /auth/mfa/verify` uçları kaldırıldı. `AuthService`'ten `hasVerifiedTotp`, `issueLoginOtp`, `requestLoginOtp`, `verifyLoginOtp`, `sessionClient`, `mfaStatus`, `mfaChallenge`, `mfaVerify` metodları silindi; `login()` eskisi gibi sadece `passwordSignIn()` + doğrudan token dönüyor (CACHE_MANAGER bağımlılığı da kaldırıldı, artık kullanılmıyor). `backend/src/auth/dto/login-otp.dto.ts`, `mfa.dto.ts`, `auth.service.spec.ts` silindi. `mail.service.ts`'ten `sendLoginOtpEmail` kaldırıldı (`sendEmailConfirmationLink` VE bugünkü IPv4/DNS düzeltmesi KORUNDU — bunlar ayrı, hâlâ geçerli özellikler/düzeltmeler). Geçici `/health/net-check` teşhis ucu kaldırıldı.
- **Web:** `components/auth/mfa-challenge-card.tsx`, `email-otp-challenge-card.tsx`, `components/settings/mfa-security-panel.tsx`, `lib/supabase-mfa.ts` silindi. `login/page.tsx` en basit haline döndü (tek adım: email+şifre → token → yönlendir). Ayarlar sayfasındaki "Güvenlik (2FA)" sekmesi tamamen kaldırıldı (artık yalnızca Görünüm/Dil sekmeleri var). `lib/supabase/client.ts`'teki `createEphemeralSupabaseClient` (yalnızca MFA tarafından kullanılıyordu) silindi. i18n'den `auth.mfa*`, `auth.otp*`, `settings.security`/`securityDesc` ve üst seviye `security.*` bloğu (tr.json/en.json) temizlendi.
- **Mobil:** `AuthStatus.mfaPending`/`otpPending`, `AuthState`'in ilgili tüm alanları (`mfaAccessToken`, `otpUserId` vb.), `submitMfaCode`/`cancelMfaChallenge`/`submitEmailOtpCode`/`resendEmailOtp`/`cancelEmailOtpChallenge`, `RegisterOutcome` enum'u kaldırıldı — `AuthNotifier.login()`/`register()` eski `Future<bool>` imzalarına döndü. `AuthRepository`'deki `LoginResult`/`RegisterResult`/`MfaStatusResult`/`MfaChallengeResult` tipleri ve ilgili tüm metodlar silindi. `login_screen.dart`'taki `_MfaChallengeCard`/`_EmailOtpChallengeCard` kaldırıldı. `app_router.dart`'a bugün eklenen mfaPending/otpPending guard'ı kaldırıldı (o state'ler artık hiç var olmadığı için gereksiz). `api_constants.dart`'tan `authMfaStatus/Challenge/Verify`, `authLoginRequestOtp/VerifyOtp` silindi; `api_client.dart`'taki `/auth/mfa/` yol kontrolü kaldırıldı. `app_strings.dart`'tan tüm `auth.mfa*`/`auth.otp*` girişleri + bugün eklenen `auth.registerSuccessLogin` temizlendi.
- **BİLİNÇLİ OLARAK DOKUNULMAYAN:** Kayıtta gerçek e-posta onayı (`register()`'ın `generateLink`/`email_confirm` mantığı) — bu iki adımlı doğrulamadan AYRI bir özellik. Ama aynı kırık SMTP altyapısına bağımlı olduğu için **hâlâ pratikte bozuk** (yeni kayıtlar onay e-postası hiç gelmediği için "waiting for verification"de takılı kalıyor) — kullanıcıya Render'da `SMTP_HOST`'u silmesi (eski otomatik-onaylı davranışa dönmek için) ayrı, bağımsız bir aksiyon olarak önerildi, bu oturumda kod tarafında ele alınmadı.

**Test/doğrulama:** Backend `npm run build`+`npm test` → **9/9 suite, 37/37 test** (2FA öncesi baseline'a birebir döndü). Frontend `npx tsc --noEmit` temiz, `npm run build` başarılı, `npx vitest run` → **3 dosya, 38/38 test**. Mobil `flutter analyze` → **0 sorun**, `flutter test` → **7/7**. Repo genelinde `mfa|MFA|TOTP|otp` için tarama yapıldı, kalan tek eşleşmeler `notifications.claimFailed` gibi tesadüfi alt-string eşleşmeleriydi (gerçek kod yok).

**Bilinen sınırlama:** Gerçek TOTP açmış bir hesap olup olmadığı bu oturumda doğrulanamadı — varsa, o hesabın Supabase'deki TOTP kaydı silinmedi (yalnızca uygulama artık ona bakmıyor), hesap artık salt şifreyle giriş yapıyor.

**Silinen dosyalar:** `backend/src/auth/dto/login-otp.dto.ts`, `backend/src/auth/dto/mfa.dto.ts`, `backend/src/auth/auth.service.spec.ts`, `frontend/src/components/auth/mfa-challenge-card.tsx`, `frontend/src/components/auth/email-otp-challenge-card.tsx`, `frontend/src/components/settings/mfa-security-panel.tsx`, `frontend/src/lib/supabase-mfa.ts`.


### [8 Eylül 2026] - Performans İyileştirmeleri ve "Ant" Rebrand (kullanıcı talebiyle)

> Kullanıcı üç ayrı iş istedi: (A) açılış/kayıt/giriş ve uygulama içi işlemlerdeki yavaşlığı **varsayımla değil kod incelemesiyle** bulup düzelt, (B) uygulamanın adını ve simgesini "Ant" (karınca) olarak değiştir (web+mobil), (C) dokümantasyon+GitHub. Performans tarafında önce bir Explore ajanıyla backend/web/mobil'in tamamı taranıp SADECE gerçekten var olan darboğazlar rapor edildi (23 Ağustos'taki disiplinin devamı) — bulunmayan yerler için zorlama "iyileştirme" YAPILMADI.

**1. Performans — gerçekten bulunan ve düzeltilen darboğazlar:**

- **Web, `frontend/src/app/actions/workspaces.ts` → `getWorkspaces()`:** Sahip olunan workspace'ler (`workspaces` tablosu) ve üye olunan workspace'ler (`workspace_members` tablosu) sorguları birbirinden bağımsız olduğu halde sıralı `await`'lerle atılıyordu. `Promise.all` ile paralelleştirildi (legacy-şema fallback sorguları, nadiren tetiklendiği ve kendi ana sorgusunun sonucuna bağlı olduğu için sıralı kaldı — bilinçli tercih). Bu fonksiyon hem giriş sonrası yönlendirmede (`resolvePostLoginRedirect`) hem her dashboard sayfası açılışında çağrılıyor.
- **Web, `frontend/src/hooks/use-workspaces.ts`:** Dashboard mount olduğunda sidebar, header, global-search-command ve workspace-presence-provider — 4 ayrı bileşen — kendi `useWorkspaces()` instance'ını kuruyor, hepsi mount'ta `getWorkspaces()` çağırıyordu; paylaşılan bir Context olmadığı için bu **4 eşzamanlı, birebir aynı, önbelleksiz** Server Action isteği anlamına geliyordu (React'ın `cache()`'i yalnızca tek bir SSR render pass'inde dedupe eder, ayrı client-side effect çağrılarını BİRLEŞTİRMEZ). Modül seviyesinde basit bir "in-flight" paylaşımı (`fetchWorkspacesShared`) eklendi: ilk çağrı gerçek isteği başlatır, aynı anda mount olan diğerleri aynı promise'i bekler — 4 istek 1'e düştü.
- **Web, `frontend/src/app/actions/notifications.ts` → `resolvePostLoginRedirect()`:** `getAuthenticatedUser()` (zaten `supabase.auth.getUser()` çağırıp doğrulamış) çağrıldıktan hemen sonra `getWorkspaces()` KENDİ İÇİNDE bağımsız olarak cookie'yi tekrar okuyup `auth.getUser()`'ı İKİNCİ KEZ çağırıyordu — aynı token için GoTrue'ya gereksiz ikinci bir doğrulama round-trip'i. `getWorkspaces()`'e opsiyonel bir `preAuth: {supabase, userId}` parametresi eklendi; `resolvePostLoginRedirect` artık zaten elindeki doğrulanmış istemci/user id'yi geçiriyor, ikinci `getUser()` çağrısı elenmiş oldu. (`getWorkspaces()`'in diğer tüm çağıranları parametre vermeden eski davranışı kullanmaya devam ediyor — geriye dönük uyumlu.)
- **Mobil, `mobile/lib/features/auth/providers/auth_provider.dart`:** `_bootstrap()`'ın soğuk başlangıçta her zaman çalışan iki bağımsız `FlutterSecureStorage.read()` çağrısı (accessToken/refreshToken, ayrı platform-channel round-trip'leri) `Future.wait` ile paralelleştirildi. Ayrıca üç yerde (bootstrap'ın temizlik dalı, `logout()`, `clearSessionLocally()`) tekrar eden üç bağımsız `delete()` çağrısı yeni ortak bir `_clearStoredTokens()` yardımcı metoduna çıkarılıp `Future.wait` ile paralelleştirildi; `_persistSession()`'daki (bazıları koşullu) üç yazma da aynı şekilde paralelleştirildi.

**Taranıp SORUN BULUNMAYAN, dolayısıyla DOKUNULMAYAN alanlar (raporun dürüstlüğü için not ediliyor):**
- **Backend guard'ları:** 23 Ağustos'ta 11 rotada bulunan "class+method seviyesinde aynı anda `@UseGuards`" bug'ı TÜM controller'larda (14 dosya, tam liste Explore raporunda) tek tek tekrar tarandı — hiçbir yerde tekrar/yeni bir örneği YOK.
- **Backend cache:** `project.controller.ts` (60sn) ve `dashboard.controller.ts` (30sn) zaten cache'li; `workspace-members.controller.ts`'in (üye listesi) 17 Ağustos'taki staleness bug'ı yüzünden BİLİNÇLİ OLARAK cache'siz bırakıldığı doğrulandı, tekrar cache eklenmedi. `task.controller.ts`'in hiç cache'i yok ama görevler workspace/üye listesinden çok daha sık değiştiği için (aynı staleness riski, daha yüksek ihtimalle) cache EKLENMEDİ — bilinçli bir "dokunma" kararı.
- **Mobil provider'lar:** task/comment/member/notification/personal/activity-log provider'larının hepsi tek tek okundu, hiçbirinde sıralı-ama-bağımsız Dio çağrısı bulunamadı (`dashboard_provider.dart`/`project_provider.dart` zaten önceki oturumlarda paralelleştirilmişti, orada da yeni bir sorun yoktu).
- **Mobil splash/bootstrap sıralaması:** workspace listesinin auth token'ı bekledikten SONRA çekilmesi bir bug değil, gerçek bir bağımlılık (Dio istemcisine token yazılmadan istek atılamaz) — doğru sıralı, dokunulmadı.

**Bilinen sınırlama — ölçüm:** Bu oturumda bulunan gerçek web darboğazlarının ikisi (hook'taki 4→1 istek paylaşımı, tarayıcı içi client-side davranış) tarayıcı DevTools olmadan ms cinsinden ÖLÇÜLEMEZ (bu makinede tarayıcı otomasyonu yok); üçüncüsü (`getWorkspaces` içindeki `auth.getUser()` round-trip'i) bir Next.js Server Action'ın içinde olduğu için düz `curl` ile de tetiklenemez (Server Action'lar özel bir RSC protokolü kullanıyor). Bu yüzden iddia SAYI bazlı değil YAPISAL: kod diff'i "4 istek → 1", "2 sıralı sorgu → paralel", "2 auth.getUser() → 1" olduğunu doğrudan gösteriyor — 23 Ağustos'taki "ms rakamı destekleyici, yapısal kanıt asıl" prensibiyle aynı çizgide. Mobildeki secure-storage paralelleştirmesi de aynı sebeple (cihaz profilleme aracı yok) ölçülemedi, yalnızca kod diff'iyle kanıtlanabilir.

**2. Rebrand — "Ant" (karınca), turuncu-siyah tema ile:**

- **Yeni simge:** `frontend/src/app/icon.svg` (SVG, Next.js App Router'ın dosya-tabanlı icon dönüşümüyle otomatik favicon/meta-icon oluyor) + `apple-icon.png` (180×180) — minimalist bir karınca silüeti (kafa/toraks/karın üç segment, 6 bacak, 2 anten), `#0a0a0a` zemin üzerinde `#ea580c` turuncu (projenin mevcut `hsl(24 100% 50%)` marka rengiyle birebir aynı aile). Eski `favicon.ico` kaldırıldı (icon.svg onun yerine geçiyor). Üretim: `sharp` (frontend'in mevcut bir bağımlılığı) ile Node script'i, harici bir araç kurulumu gerekmedi.
- **Web metinleri:** `app/layout.tsx` (`<title>` → "Ant"), auth split-shell kicker etiketi ("staj-projesi" → "Ant"), onboarding kicker etiketi ("İş Yönetim Sistemi" → "Ant"), `backend/src/main.ts` Swagger başlığı/açıklaması ("staj-projesi API" → "Ant API").
- **Mobil:** `pubspec.yaml` description güncellendi (paket adı `mobile` olarak KORUNDU — Dart paket kimliği, kullanıcıya görünmüyor, değiştirmek geniş bir iç-kod riski olurdu, gerekçesiz). `AndroidManifest.xml`'de `android:label` → "Ant". `Info.plist`'te `CFBundleDisplayName`/`CFBundleName` → "Ant". `main.dart`'taki `MaterialApp.router(title: ...)` → "Ant". `auth_split_shell.dart` ve `home_screen.dart`'taki (drawer başlığı) sabit "STAJ-PROJESI" metinleri → "ANT".
- **Mobil ikon üretimi:** `flutter_launcher_icons` (^0.14.3) dev_dependency olarak eklendi, `mobile/assets/icon/icon.png` (1024×1024, aynı karınca tasarımı) kaynak olarak verilip `dart run flutter_launcher_icons` ile hem Android (5 mipmap yoğunluğu) hem iOS (`Assets.xcassets/AppIcon.appiconset/`, eksik olan birkaç legacy boyut dahil tam set) ikonları TEK komutla otomatik üretildi.
- **`DESIGN.md`:** Proje başlığı "İş Yönetim Sistemi (Workspace App)" → "Ant — İş Yönetim Sistemi (Workspace App)".
- **`anlatim.md` BULUNAMADI:** Kullanıcı bu dosyaya da rebrand yansıtılmasını istemişti ama dosya bu oturum başlamadan ÖNCE zaten repodan silinmiş durumdaydı (`git status` bunu bir önceki oturumdan kalma bekleyen bir silme olarak gösteriyor, bu oturumun işi değil) — güncellenecek bir içerik yok, kullanıcıya açıkça bildiriliyor.

**Test/doğrulama:**

- Backend: `npm run build` + `npm test` → **9/9 suite, 37/37 test**.
- Frontend: `npx tsc --noEmit` temiz, `npm run build` başarılı (`/icon.svg` ve `/apple-icon.png` rotaları doğru üretildi), `npx vitest run` → **3 dosya, 38/38 test**.
- Mobil: `flutter analyze` → **0 sorun**, `flutter test` → **7/7**.
- **Cihaz testi:** `flutter devices` ile 2201116TG (Android 11) bağlı bulundu. `flutter build apk --release --dart-define=API_BASE_URL=https://staj-projesi-api.onrender.com` ile yeni ikon/isimle bir release APK derlendi (58.0MB), `flutter install -d OVSCCI8TSCZD9PXW` ile cihaza kuruldu (eski sürüm otomatik kaldırılıp yenisi kuruldu, hatasız tamamlandı). **Bilinen sınırlama:** bu oturumda ekrana dokunup ana ekranda yeni ikonun/ismin gerçekten göründüğünü GÖRSEL olarak doğrulayacak bir araç yoktu (dokunmatik/screenshot yeteneği yok) — yalnızca derleme+kurulumun hatasız tamamlandığı kanıtlanabildi. Kullanıcının cihazda elle bakması öneriliyor.
- **iOS:** İkon seti (15+6 = 21 dosya, `Contents.json` dahil) `flutter_launcher_icons` ile eksiksiz güncellendi, ama önceki oturumlarda da not edildiği gibi bu projede hâlâ bir `Podfile` yok — iOS tarafı pratikte hiç build edilmemiş/kullanılmamış durumda, bu oturumda da gerçek bir iOS build denenmedi (kapsam dışı, yalnızca statik asset güncellemesi yapıldı).

**Yeni dosyalar:** `frontend/src/app/icon.svg`, `frontend/src/app/apple-icon.png`, `mobile/assets/icon/icon.png`.
**Silinen dosyalar:** `frontend/src/app/favicon.ico`.
